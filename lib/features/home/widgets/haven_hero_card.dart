...
import '../../engine/safe_to_spend_why_sheet.dart';
...
/// Layer 2 — Safe to Spend, visually dominant (PD-038 confidence states).
class _SafeToSpendLayer extends StatelessWidget {
  ...
  Widget _copyFor(SafeToSpendResult result) {
    switch (result.state) {
      case SafeToSpendState.confident:
        final amount = result.displayAmount ?? 0;
        return Column(
          crossAxisAlignment: CrossAxisAlignment.start,
          children: [
            Text(
      ...
      case SafeToSpendState.estimated:
        ...
        return Column(
          ...
          children: [
            Text(
              'Around ${HavenTypography.formatAmount(amount)}',
              style: HavenTypography.h1.copyWith(fontSize: 28),
            ),
            const SizedBox(height: HavenSpacing.xs),
            Text(
              'Based on what Haven currently knows.',
              style: HavenTypography.bodySmall.copyWith(
                color: HavenColors.textSecondary,
              ),
            ),
      ...
      case SafeToSpendState.unknown:
        return Column(
          ...
          children: [
            Text(
              'Haven needs a little more information before it can calculate Safe to Spend.',
              style: HavenTypography.body.copyWith(
                color: HavenColors.textPrimary,
                fontWeight: FontWeight.w600,
                height: 1.4,
            ...
            if (result.missingHints.isNotEmpty) ...[
              const SizedBox(height: HavenSpacing.sm),
              Text(
                result.missingHints.take(2).join(' · '),
